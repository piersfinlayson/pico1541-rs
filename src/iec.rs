...
use embassy_rp::gpio::{Input, Output, Level, Pull, Pin};
...
impl IecBus {
    ...
    pub fn new(
        clock_in: impl Pin, clock_out: impl Pin,
        data_in: impl Pin, data_out: impl Pin,
        srq_in: impl Pin, srq_out: impl Pin,
        atn_in: impl Pin, atn_out: impl Pin,
        reset_in: impl Pin, reset_out: impl Pin,
    ) -> Self {
        let mut bus = Self {
    ...
    pub fn set_lines(&mut self, mask: u8) {
        if (mask & IO_DATA) != 0 { self.set_data(); }
        if (mask & IO_CLK) != 0 { self.set_clock(); }
        if (mask & IO_ATN) != 0 { self.set_atn(); }
        if (mask & IO_RESET) != 0 { self.set_reset(); }
        if (mask & IO_SRQ) != 0 { self.set_srq(); }
    }
    
    /// Release multiple lines at once based on a bit mask
    pub fn release_lines(&mut self, mask: u8) {
        if (mask & IO_DATA) != 0 { self.release_data(); }
        if (mask & IO_CLK) != 0 { self.release_clock(); }
        if (mask & IO_ATN) != 0 { self.release_atn(); }
        if (mask & IO_RESET) != 0 { self.release_reset(); }
        if (mask & IO_SRQ) != 0 { self.release_srq(); }
    }
    ...
    pub fn poll_pins(&self) -> u8 {
        let mut result = 0;
        if !self.get_data() { result |= IO_DATA; }
        if !self.get_clock() { result |= IO_CLK; }
        if !self.get_atn() { result |= IO_ATN; }
        if !self.get_reset() { result |= IO_RESET; }
        if !self.get_srq() { result |= IO_SRQ; }
        result
    }
    ...
    pub fn iec_to_hw(&self, iec: u8) -> u8 {
        let mut hw = 0;
        if (iec & IEC_DATA) != 0 { hw |= IO_DATA; }
        if (iec & IEC_CLOCK) != 0 { hw |= IO_CLK; }
        if (iec & IEC_ATN) != 0 { hw |= IO_ATN; }
        if (iec & IEC_SRQ) != 0 { hw |= IO_SRQ; }
        hw
    }
    ...
    pub fn iec_poll(&self) -> u8 {
        ...
        if (pins & IO_DATA) != 0 { iec |= IEC_DATA; }
        if (pins & IO_CLK) != 0 { iec |= IEC_CLOCK; }
        if (pins & IO_ATN) != 0 { iec |= IEC_ATN; }
        if (pins & IO_SRQ) != 0 { iec |= IEC_SRQ; }
